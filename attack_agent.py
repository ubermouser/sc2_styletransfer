...
# Adapted from https://github.com/skjb/pysc2-tutorial/blob/master/Building%20an%20Attack%20Agent/attack_agent.py
...
BUILT_UNIT_REWARD = 0.05
KILL_UNIT_REWARD = 0.2
KILL_BUILDING_REWARD = 0.5

DATA_FILE = "agent_weights.pds"
...
class AttackAgent(PredictorAgent):
#class AttackAgent(base_agent.BaseAgent):
    def __init__(self):
        ...
        self.previous_built_unit_score = 0
    ...
    def step(self, obs):
        ...
        has_supply = 1 if depot_y.any() else 0

        barracks_y, barracks_x = (unit_type == _TERRAN_BARRACKS).nonzero()
        has_barracks = 1 if barracks_y.any() else 0

        food_available = obs.observation['player']['food_cap'] - obs.observation['player']['food_used']
        ...
        built_unit_score = obs.observation['score_cumulative']['total_value_units']
        
        current_state = np.zeros(20)
        current_state[0] = has_supply
        current_state[1] = has_barracks
        current_state[2] = food_available
        current_state[3] = 0
        ...
        current_state[4:] = hot_squares[:]

        if self.previous_action is not None:
            game_reward = obs.observation['score_cumulative']['score']
            reward = 0

            if built_unit_score > self.previous_built_unit_score:
                reward += BUILT_UNIT_REWARD
        ...
        self.previous_built_unit_score = built_unit_score
